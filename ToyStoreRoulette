...
class ToyStore {
    ...
    public void drawPrizes(int numPrizes) {
        ...
        while (numPrizes > 0 && numToys > 0) {
            ...
            if (chosenToy.getQuantity() == 0) {
                System.out.println(chosenToy.getName() + " закончились.");
                break;
            }
    ...
    public void loadPrizes(String fileName) throws IOException {
        this.fileName = fileName;
        ...
        for (String line : lines) {
            ...
            String name = parts[1];
            prizes.add(new Toy(id, name, 1, 1, 0));
            if (id >= currentPrizeId) {
                currentPrizeId = id + 1;
            }
        }
    }

    public Optional<Toy> getNextPrize() {
        if (prizes.isEmpty()) {
        ...
        Toy nextPrize = prizes.get(prizeIndex);
        prizeIndex++;
        return Optional.of(nextPrize);
    }
}
